...
const [listeCreated, setListeCreated] = useState(false);
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const element = seanceData.seanceInstance.element.libelleElement;
        console.log("hhhhhhhhhhhhhhhhlaiaiaiai",etudiants)
                ...
                <table>
                  <thead>
                    <tr>
                      <th>CNE</th>
                      <th>Nom</th>
                      <th>Présent ${heureDebut}-${heureFin}</th>
                    </tr>
                  </thead>
                  <tbody id="tbody">`
    students.forEach(element => {
      ...
      htmlContent += `
            ...
            <td>${ (element.nom=="MANDOUR")?'non':'oui'}</td>
            </tr>
            `
    });
  ...
  return (
    <div className="d-flex flex-column align-items-center" style={{ height: '100vh' }}>
      ...
      <br />
      <br />
      <h1 className="text-center mb-4">prof : ELBOUJNOUNI , seance de :Cryptographie </h1>
